...
def test__inverser_lignes():
 ...
 assert resultat == plateau_attendu, f"j'aurai du avoir ce plateau {plateau_attendu} mais obtenu {resultat}"
 print("OK")
...
def test__deplacer_droite():
 ...
 [0, 0, 4, 4],
 [0, 0, 0, 8],
 [0, 8, 4, 4] # Attention ici, vérifie bien l'ordre de fusion !
 ]
 attendu_pts = 24

 # CORRECTION ICI : on appelle bien _deplacer_droite
 resultat, points = _deplacer_droite(plateau)
 
 assert resultat == attendu_p, f"ERREUR : obtenu {resultat} au lieu de {attendu_p}"
 assert points == attendu_pts
 print("OK")
...
def test__transposer():
 ...
 # On compare le résultat au plateau_attendu
 assert resultat == plateau_attendu, f"ERREUR : obtenu {resultat} au lieu de {plateau_attendu}"
 print("OK")
...
def test__deplacer_haut():
 ...
 # CORRECTION : on récupère la grille ET le score
 resultat, score = _deplacer_haut(grille)
 
 # On compare uniquement la grille avec ce qui est attendu
 assert resultat == attendu, f"j'ai obtenu {resultat} mais devrait avoir {attendu}"
 
 # Optionnel : vérifier que le score est bien 4 (2+2)
 assert score == 4
